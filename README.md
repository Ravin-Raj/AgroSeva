# 🌾 AgroSeva – Mobile Application for Farm Management

**AgroSeva** is a mobile-based farm management application built using **Android Studio (Java & XML)**.  
It helps farmers enhance productivity by managing crop cultivation efficiently with tools for soil analysis, irrigation scheduling, labour management, market tracking, and profit/loss calculation — all within a user-friendly mobile interface.

---

## 📱 Overview

AgroSeva is designed to modernize traditional farming practices, specifically focusing on **tomato cultivation**.  
It provides a structured digital platform where farmers can:
- Plan and track cultivation stages.
- Monitor soil health and weather conditions.
- Receive water reminders and notifications.
- Manage workforce and finances.
- Access real-time market rates and offers.

The app supports both **English and Tamil** languages, ensuring accessibility for local farmers.

---

## 🛠️ Tech Stack

| Category | Technology Used |
|-----------|-----------------|
| **IDE** | Android Studio |
| **Language** | Java, XML |
| **Backend** | Firebase Realtime Database |
| **API** | OpenWeatherMap (for weather data) |
| **Testing Tool** | Android Emulator |
| **Operating System** | Windows 11 |
| **Database Storage** | Cloud (Firebase) |

---

## 🧩 Core Modules

### 1. 👤 Signup & Login
- Secure user authentication system.  
- “Forgot Password” option for recovery.

### 2. 🏠 Home Page
- Categorized layout for **Vegetables** and **Fruits** modules.  
- Easy navigation for all tools.

### 3. 🍅 Tomato Farming Module
- Step-by-step cultivation guide.  
- Soil testing (pH, NPK analysis) and fertilizer recommendations.  
- Water reminders for efficient irrigation scheduling.  
- Labour tracking with salary and task management.  
- Profit/Loss calculator for financial insights.

### 4. 🌦️ Weather API Integration
- Fetches real-time weather forecasts for Tamil Nadu districts.  
- Assists in planning irrigation and protecting crops.

### 5. 💬 Language Translator
- Supports **Tamil ↔ English** translation for localized usability.

### 6. 💰 Market Rate & Offers
- Displays **daily crop prices** from verified sources.  
- Provides notifications for government schemes and discounts.

---

## ⚙️ System Requirements

### Hardware
- RAM: 16 GB  
- Processor: Intel Core i5 (13th Gen)  
- Storage: 512 GB SSD  
- Display: 15.1” Color Monitor  

### Software
- OS: Windows 11 Home (64-bit)  
- IDE: Android Studio  
- JDK: Java 8+  
- Emulator: Android Emulator  
- Cloud: Firebase  

---

## 🧪 Testing Summary

| Type | Description |
|------|--------------|
| **Functional Testing** | Verified all features (Login, Weather, Translator, Profit Module). |
| **Usability Testing** | Ensured smooth and intuitive navigation. |
| **Performance Testing** | Tested stability under high data load. |
| **Compatibility Testing** | Validated app performance across devices and Android versions. |
| **Security Testing** | Protected user data and Firebase integration. |
| **Beta Testing** | Conducted with farmers to gather real-world feedback. |

---

## 🚀 Key Features
- 📊 Real-time market price tracking  
- 🧪 Soil and fertilizer testing  
- ⏰ Watering reminders with notifications  
- 🌦️ Live weather forecasting  
- 👨‍🌾 Labour and expense management  
- 💰 Automated profit/loss calculation  
- 🈁 Bilingual interface (Tamil & English)  

---

## 🔮 Future Enhancements
- AI-powered **crop recommendations**  
- **Drone integration** for pest detection and field monitoring  
- **Blockchain-based supply chain tracking**  
- **Offline support** for rural areas  
- Integrated **E-Marketplace** for direct farmer–buyer sales  
- **Community forum** for farmer collaboration  
- Financial tools like **loan calculators** and **subsidy tracking**  

---

## 🧾 Conclusion
**AgroSeva** bridges the gap between agriculture and technology, providing farmers with a digital companion for smart farming.  
It empowers users to:
- Increase crop yield  
- Reduce operational costs  
- Make data-driven agricultural decisions  

This project showcases how technology can drive **sustainable, efficient, and profitable farming** for the next generation.

---

## 👨‍💻 Developers
- **S. Ravin Raj (23322009)**  
- **E. Nivetha (23322022)**  

**Department of Computer Science and Applications**  
**The Gandhigram Rural Institute (Deemed to be University)**  
*December 2024*

---

## 📚 References
- [Android Studio Documentation](https://developer.android.com/studio)  
- [Firebase Realtime Database](https://firebase.google.com/docs/database)  
- [OpenWeatherMap API](https://openweathermap.org/api)  
- [GeeksforGeeks Android Tutorials](https://www.geeksforgeeks.org/android-tutorial/)  
- [Stack Overflow Discussions](https://stackoverflow.com/questions/tagged/android)

---

## 🪪 License
This project is open for educational and research use under the **MIT License**.
